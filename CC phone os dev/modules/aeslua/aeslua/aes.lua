...
local gf = require("modules.aeslua.aeslua.gf");
local util = require("modules.aeslua.aeslua.util");
...
function private.affinMap(byte)
    ...
    for i = 1,8 do
        ...
        mask = bit.band(bit.brshift(mask, 1),0xff);
        if (lastbit ~= 0) then
...
function private.rotWord(word)
    ...
    return (bit.blshift(word,8) + bit.brshift(tmp,24)) ;
end
